...
def fortran_complete():
    ...
    syntax_elements = [
        ...
        SyntaxElement(re.compile(r'^\s*module\s+(?P<name>[a-zA-Z0-9_]+)'),
                      'end module' ),
        SyntaxElement(re.compile(r'^.*?subroutine\s+(?P<name>[a-zA-Z0-9_]+)'),
                      'end subroutine' ),
        SyntaxElement(re.compile(r'^.*?function\s+(?P<name>[a-zA-Z0-9_]+)'),
                      'end function' ),
    ...
    for syntax_element in syntax_elements:
        ...
        if closingline is not None:
            ...
            vim.command('nohls') #clear search highlight
...
nmap <F12> :python3 fortran_complete()<cr>A
imap <F12> :python3 fortran_complete()<cr>A
